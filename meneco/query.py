    ...
    for a in termset:
        if a.pred() == "reaction":
            ...
            if a.arg(0) in dict:
                ...
                print("Error: unknown ireaction, query.py line 64")
...
def get_minimal_completion_size(draft, repairnet, seeds, targets):
    ...
    instance = TermSet(
        draft.union(draftfact).union(repairnet).union(targets).union(seeds)
    )
...
def get_intersection_of_optimal_completions(draft, repairnet, seeds, targets, optimum):
    ...
    options = (
        "--configuration jumpy --opt-strategy=5 --enum-mode cautious --opt-mode=optN --opt-bound="
        + str(optimum)
    )
...
def get_union_of_optimal_completions(draft, repairnet, seeds, targets, optimum):
    ...
    options = (
        "--configuration jumpy --opt-strategy=5 --enum-mode brave --opt-mode=optN --opt-bound="
        + str(optimum)
    )
...
def get_optimal_completions(draft, repairnet, seeds, targets, optimum, nmodels=0):
    ...
    options = (
        str(nmodels)
        + " --configuration jumpy --opt-strategy=5 --opt-mode=optN --opt-bound="
        + str(optimum)
    )
    solver = Gringo4Clasp(clasp_options=options)
    models = solver.run(prg, collapseTerms=True, collapseAtoms=False)
